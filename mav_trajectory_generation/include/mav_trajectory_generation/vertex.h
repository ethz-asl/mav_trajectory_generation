...
class Vertex {
 public:
  ...
  /// values in the constraint vector. The dimension has to match the
  /// derivative.
  void addConstraint(int type, const Eigen::VectorXd& constraint);

  /// Removes a constraint for the derivative specified in type. Returns false
  /// if constraint was not set.
...
/// The time_factor \in [1..Inf] increases the allocated time making the
/// segments slower and thus feasibility more likely. This method does not take
/// into account the start and goal velocity and acceleration.
...
/// exp(- distance/v_max * 2); magic_fabian_constant was determined to 6.5 in a
/// student project ...
...
/// derivative set to zero, while  all vertices in between have position as
/// fixed constraint and the derivatives are left free. Input:
/// maximum_derivative = The maximum derivative to be set to zero for beginning
/// and end. Input: n_segments = Number of segments of the resulting trajectory.
/// Number of vertices is n_segments + 1. Input: minimum_position = Minimum
/// position of the space to sample. Input: maximum_position = Maximum position
/// of the space to sample. Input: seed = Initial seed for random number
/// generation. Output: return = Vector containing n_segments + 1 vertices.
